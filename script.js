...
function createBubble(numBubbles) {
  for (let i = 0; i < numBubbles; i++) {
    const bubble = document.createElement('div');
    bubble.classList.add('bubble');
    bubble.style.left = Math.random() * window.innerWidth + 'px';
    bubble.style.top = Math.random() * window.innerHeight + 'px'; // Initial random position
    bubble.style.animationDelay = Math.random() * 3 + 's'; // Randomize animation delay
    bubbleContainer.appendChild(bubble);
  }
}

const randomBubbleCount = Math.floor(Math.random() * 6) + 10;
createBubble(randomBubbleCount);
